...
export default function DeliveriesManagementPage() {
  const deliveries = [
    { commande: 'CMD-001', client: 'Jean Dupont', adresse: '123 Rue de la Paix, 75000 Paris', livreur: 'Luc Martin', heure: '2025-01-15 20:15', status: 'Livrée' },
    { commande: 'CMD-003', client: 'Pierre Martin', adresse: '45 Avenue des Champs-Élysées, 75008 Paris', livreur: 'Sophie Laurent', heure: '2025-01-15 13:30', status: 'En route' },
    { commande: 'CMD-005', client: 'Marie Dubois', adresse: '78 Boulevard Saint-Germain, 75006 Paris', livreur: 'Paul Moreau', heure: '2025-01-15 19:45', status: 'En préparation' },
    { commande: 'CMD-006', client: 'Thomas Bernard', adresse: '34 Rue de Rivoli, 75004 Paris', livreur: 'Emma Petit', heure: '2025-01-14 21:00', status: 'Annulée' },
  ];
                ...
                <table className="min-w-full">
                  <thead>
                    <tr className="border-b border-[--color-accent]/20 bg-gradient-to-r from-[--color-accent]/10 to-transparent">
                      ...
                      <th className="py-4 px-6 text-left text-sm font-semibold text-[--color-accent] uppercase tracking-wider">Heure de livraison</th>
                  ...
                  <tbody className="divide-y divide-[--color-accent]/10">
                    {deliveries.map((delivery, index) => (
                      <tr key={index} className="hover:bg-[--color-accent]/5 transition-all duration-200 group">
                        ...
                        <td className="py-4 px-6 text-sm text-[--color-text-muted] max-w-xs">{delivery.adresse}</td>
                        <td className="py-4 px-6 text-sm text-[--color-text-muted]">{delivery.livreur}</td>
                        <td className="py-4 px-6 text-sm text-[--color-text-muted]">{delivery.heure}</td>
